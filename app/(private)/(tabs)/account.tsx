...
import { getProfileDetails } from "~/lib/queries/profile.query";
  ...
  return (
    <View className="flex-1 bg-neutral-50">
      <View className="py-7 items-center gap-5">
        <View className="p-1 bg-neutral-50 border border-neutral-300 rounded-full">
          <View className="size-24 bg-neutral-100 rounded-full overflow-hidden items-center justify-center">
            {data && data.avatar ? (
              <Image 
                className='size-full' 
                source={AVATARS[data.avatar]} 
              />
            ): null}
          </View>
        </View>
        <View>
          <Text className="text-xl font-normal text-neutral-800">
            {data?.username}
          </Text>
        </View>
      </View>
      ...
      <View className="mt-7">
        <Text className="text-center text-sm text-neutral-500 font-normal">Tazzy v1.0.0 — Beta</Text>
      </View>
      <BottomSheetModal
        ref={sheetRef}
      ...
      >
        <BottomSheetView className="pt-5 pb-5">
          <View className="px-5">
            <Text className="text-lg font-medium text-neutral-700 text-center">Select a theme</Text>
          </View>
          <View className="py-5">
