...
export default function TabAddButton() {
  ...
  const taskX = useSharedValue(0);
  const taskY = useSharedValue(0);
  const schedX = useSharedValue(0);
  const schedY = useSharedValue(0);
  ...
  const toggleAdd = () => {
    ...
    taskX.value = withTiming(isOpening ? -48 : 0, { duration: 200 });
    taskY.value = withTiming(isOpening ? -64 : 0, { duration: 200 });
    schedX.value = withTiming(isOpening ? 48 : 0, { duration: 200 });
    schedY.value = withTiming(isOpening ? -64 : 0, { duration: 200 });
  ...
  return (
    <View className='relative'>
      ...
      <Animated.View style={[plusStyle]} className="relative">
        <Pressable
          className="h-16 aspect-square mx-2 bg-[#0284c7] rounded-full flex items-center justify-center"
          onPress={toggleAdd}
        >
      ...
      {/* Add Task Button */}
      <Animated.View style={[taskStyle]} className="absolute">
        <Pressable
          className="relative h-16 aspect-square mx-2 bg-[#0284c7] rounded-full flex items-center justify-center"
      ...
      {/* Add Schedule Button */}
      <Animated.View style={[schedStyle]} className="absolute">
        <Pressable
          className="h-16 aspect-square mx-2 bg-[#0284c7] rounded-full flex items-center justify-center"
